				...
				if($pending > 0 && $div[divisi]=='MAINTENANCE' && $div[level]=='superadmin'){
				 ...
				 <div class="card top_counter">
                        <div class="body">
							...
							<div class="content">
								<div class="text"><b><font color='red'>Pending Masalah</font></b></div>
								<h1 class="number">
								<?php
									...
									<?php
										...
										while ($data = mysqli_fetch_assoc($ptampil))
										{
											if($div[level]=='superadmin'){
												if($data[status_problem]=='OPEN'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]";}
												if($data[status_problem]=='ASSIGN'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&tek=$data[PIC_HANDLING]";}
												if($data[status_problem]=='IN PROGRESS'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&tek=$data[PIC_HANDLING]";}
												if($data[status_problem]=='MENUNGGU SPAREPART'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&s=report&tek=$data[PIC_HANDLING]";}
												if($data[status_problem]=='FINISH'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&s=app&tek=$data[PIC_HANDLING]";}
												if($data[status_problem]=='APPROVED'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&s=report&tek=$data[PIC_HANDLING]&s=close";}
												if($data[status_problem]=='PENDING'){$kirim = "?p=todolist&act=problem-detail&id=$data[idprob]&tek=$data[PIC_HANDLING]&s=app";}
											}else{
											...
											<tr>
												...
												if($data[category_name] == 'Kritis'){
													echo"
														...
														<td bgcolor='red'><font color='white'><a href='$kirim'><b>$data[idprob]</b></a></font></td>
													...
													echo"	
														...
														<td bgcolor='yellow'><a href='$kirim'><b>$data[idprob]</b></a></td>
													...
													echo"
														...
														<td><a href='$kirim'><b>$data[idprob]</b></a></td>
