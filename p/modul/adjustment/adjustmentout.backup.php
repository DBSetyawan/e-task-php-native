                                ...
                                <table class="table table-hover js-basic-example dataTable table-custom m-b-0" border='1'>
									...
									*/
									...
									for($i=0; $i<$jumlah; $i++)  
									{
										$urut =  $_POST['adj'][$i];
										//echo $urut."<br>";
										/* $request_query="SELECT t.idReq, t.idProb, p.status_problem,
										...
										//$idreq = $_POST['req'][$nourut];
										/* $data_request= mysqli_fetch_array(mysqli_query($conn,$request_query));

										$nourut=$data_request['idreq'];
										$idreq=$data['']; */


										/* $cari = "SELECT t.idReq, t.idProb, t.nama_teknisi, t.mesin, t.unit, t.kode_sparepart, t.mesin, t.unit, ta.nama_sparepart, t.qty, t.satuan, t.createdDate, ta.info, ta.createdBy as appBy, ta.createdDate as appDate 
												 FROM tsparepart t left join tsparepart_action ta ON t.idReq=ta.idReq
												 where ta.createdBy IS NOT NULL AND ta.createdDate IS NOT NULL AND 
												 t.idReq=$idreq " ; */
										...
										/* $kodeetask=$r['idProb'];
										$koderequest=$r['idReq']; */
										/* echo "<input name='etask[]' value='$kodeetask' style='display:none;'>";
										echo "<input name='req[]' value='$koderequest' style='display:none;'>"; */
									}
									$tdata=$data;
									sort($tdata);
									sort($data);
									...
									echo "<br>";
									echo "<br>";*/
									//proses pengelompokan materialcode yang sama dan menjumlahkan qty nya
									/*for ($k=0; $k <$jumlah ; $k++) { 
										if ($k>0) {
											for ($j=0; $j < $k; $j++) { 
													//$totalsama=0;
													if ($tdata[$j]['code'] == $tdata[$k]['code']) {
														$tempcode= $tdata[$j]['code'];
														$tempqty = $tdata[$j]['qty'] + $tdata[$k]['qty'];
														$data[$j]['qty']=$tempqty;
														array_splice($tdata, $k, 1);
														...
														if ($totalsama==0) {
															...
															//agar jumlah item ditambahkan ke item terdepan saja
															$totalsama++;	
														}
													}
												}
														...
														*/
														...
														?>
															<tr>
																...
																<input name='etask[]' value='<?php echo $kodeetask; ?>' style='display:none;'>
																<input name='req[]' value='<?php echo $koderequest; ?>' style='display:none;'>
									                        </tr>
														<?php
														$dataqty=0;	
														$no++;
