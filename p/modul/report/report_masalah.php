								...
								<tr><td width='15%'>Kode Masalah</td><td>";?>
									<select id="kota2" name="idprob[]" class="theSelect form-control" multiple="multiple">
				                        <option value=""></option>
				                           ...
				                           if($u[divisi]=='GA'){
					                        	$query2 = "SELECT idProb from tproblems WHERE LEFT(idProb,3) = 'PGA' order by idProb ASC;";
					                    	}
					                    	else{
					                    		$query2 = "SELECT idProb from tproblems WHERE LEFT(idProb,3) = 'TSK' order by idProb ASC;";
					                    	}
									...
									<tr><td width='15%'>Teknisi </td><td>
										<select class='default-select2 form-control' name='nm_teknisi'>
											...
											$q = mysqli_query($conn, "select FULLNAME,USERNAME from user where divisi='$u[divisi]' and (active=1 or active=2) order by fullname asc");
