							...
							<div class='table-responsive'>
                                <table class='table table-striped table-hover dataTable js-exportable'>
									...
									$cari = "SELECT tp.idprob as cdprob, tp.namapelapor, tp.dateprob, tp.timeprob, tp.deskripsi, tp.created_by, tp.divisi_problem,
											...
											if($u2[divisi]!="MAINTENANCE" OR $u2[divisi]!="GA"){
												$cari .= " AND tp.divisi_problem = '$u2[divisi]'";
											}if($u2[divisi]=="MAINTENANCE"){
												$cari .= " AND tp.idprob LIKE 'TSK%'";
											}if($u2[divisi]=="GA"){
												$cari .= " AND tp.idprob LIKE 'PGA%'";
								...
								$at = mysqli_query($conn, "SELECT idprob, `no`, statusProblem, dateAction FROM thandling t where statusProblem = 'PAUSED' and idprob='$r[cdprob]' order by idprob asc");
