...
<div class='block-header'>
                        ...
                        <div class='body'>
                            <div class='row clearfix'>                               
                                <div class='col-sm-12'>";
										...
										$t = mysqli_fetch_array(mysqli_query($conn,"select  year(NOW())y, (DATE_FORMAT(CURDATE(),'%m'))m, (DATE_FORMAT(CURDATE(),'%d'))d,max(idprob) as no , dateprob 
											from tproblems where dateprob=date(NOW()) and idprob like '".$_GET['series']."%' "));
										
										$ye = $t['y']; $mo = $t['m']; $da = $t['d'];
										$noUrut = (int) substr($t[no], 13, 3);
										$noUrut++;
										//$series_dep = $docseries['series'];
										$series_dep = $_GET['series'];
										$char = "$series_dep-$ye$mo$da-";
									...
									echo "<table border='0' width='100%'>
										...
										echo "	
											...
											<tr><td colspan='2'>
											 <font color='red' size='2'><b>(Personal and GA)</b></font><br>Priority Notess :</td></tr>
			...
			$dated = date('Y-m-d');
						$docseries = mysqli_fetch_array(mysqli_query($conn,"SELECT series, dokumen, divisi FROM mdocumentseries m where dokumen like 'TSK%' AND divisi = '$u2[divisi]'"));
										...
										$newID = $char . sprintf("%03s", $noUrut);
						echo "
						<input type='hidden' name='in_ke' value='internal'>
                            ...
                            <div class='row clearfix'>                               
                                <div class='col-sm-12'>
									...
									<select class='form-control show-tick' name='idmesin' id='idmesin' required>
										...
										$cekm = mysqli_query($conn, "SELECT a.series, a.divisi, m.kategori FROM mdocumentseries a
																	   left join mdocumentseriesmt m on a.series=m.series
																	   where a.series='$_GET[series]' AND a.dokumen like 'TSK%';");
										while($rr = mysqli_fetch_array($cekm)){
											$r = mysqli_query($conn, "select *from tmesin where kategori = '$rr[kategori]' order by namaMesin ASC");
											while($c = mysqli_fetch_array($r)){
